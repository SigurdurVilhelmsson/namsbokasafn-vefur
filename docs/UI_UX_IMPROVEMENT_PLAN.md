...
- [~] **QW-2:** "Copy citation" button for equations and figures — ⚠️ Backend handler exists in `equations.ts`, but content pipeline does not generate the `data-action="copy-citation"` buttons in HTML output. Non-functional.
- [x] **QW-3:** Keyboard shortcut overlay (toggle with `?`) showing all available shortcuts
- [ ] **QW-4:** Sound effects for flashcard success — ⚠️ **Settings toggle exists but no audio implementation.** The `soundEffects` boolean is stored and toggled in `SettingsModal.svelte`, but no `sounds.ts` utility, audio files, or Web Audio API calls exist. The toggle is a dead control.
...
**Status:** Pending — confirmed the problem: mobile h1 and h2 are both 1.5rem, making them visually indistinguishable
...
1. **Fix QW-4: Implement actual sound effects** — The settings toggle exists but does nothing. Either implement Web Audio API sounds (subtle click/success tones) or remove the dead toggle from settings to avoid confusing users.
2. **Fix QW-2: Wire up citation copy buttons** — The `copyCitation()` handler in `equations.ts` is ready, but the content pipeline doesn't generate the trigger buttons. Either add button injection in the equation action or coordinate with the content pipeline in namsbokasafn-efni.
3. **Fix 4.1: Mobile heading hierarchy** — h1 and h2 are both 1.5rem on mobile, making them visually identical. Quick CSS fix.
