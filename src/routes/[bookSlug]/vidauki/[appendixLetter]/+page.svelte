...
<article class="max-w-3xl mx-auto px-1 sm:px-0">
	...
	<nav class="mb-6 text-sm text-gray-500 dark:text-gray-400" aria-label="Brauðmylsna">
		<ol class="flex items-center gap-2">
			<li>
				<a href="/{data.bookSlug}" class="hover:text-[var(--accent-color)]">
					Efnisyfirlit
				</a>
			</li>
	...
	<div class="flex flex-col sm:flex-row items-stretch gap-4 border-t border-gray-200 dark:border-gray-700 pt-6">
		...
		{#if data.nextAppendix}
			<a
				...
				class="flex-1 group p-4 rounded-xl border border-gray-200 dark:border-gray-700 hover:border-[var(--accent-subtle)] hover:bg-[var(--accent-light)] transition-colors text-right"
			>
			...
			{#each data.allAppendices as appendixItem}
				<li>
					<a
						...
						class="flex items-center gap-3 p-3 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors {appendixItem.letter === data.appendix.letter ? 'bg-[var(--accent-light)]' : ''}"
