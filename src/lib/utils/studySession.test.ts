...
describe('findUnreadSections', () => {
	...
	it('returns unread content sections, skipping non-content types', () => {
		...
		const unread = findUnreadSections(basicToc, progress, 'efnafraedi-2e', null);
	...
	it('excludes already-read sections', () => {
		const progress: ReadingProgress = {
			'efnafraedi-2e/01/1-1': { read: true, lastVisited: '2025-01-01' }
		};
	...
	it('filters by chapter when chapterFilter is set', () => {
		...
		const unread = findUnreadSections(basicToc, progress, 'efnafraedi-2e', null, 2);
	...
	it('prioritizes current chapter', () => {
		...
		const unread = findUnreadSections(basicToc, progress, 'efnafraedi-2e', '02');
	...
	it('returns empty array when all sections are read', () => {
		const progress: ReadingProgress = {
			'efnafraedi-2e/01/1-1': { read: true, lastVisited: '2025-01-01' },
			'efnafraedi-2e/01/1-2': { read: true, lastVisited: '2025-01-01' },
			'efnafraedi-2e/02/2-1': { read: true, lastVisited: '2025-01-01' },
			'efnafraedi-2e/02/2-2': { read: true, lastVisited: '2025-01-01' }
		};
